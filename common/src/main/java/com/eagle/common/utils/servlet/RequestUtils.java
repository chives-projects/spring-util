package com.eagle.common.utils.servlet;
...
public class RequestUtils {
    ...
    public static String getClientIp() {
        if (isServletContext()) {
            return getClientIp(RequestUtils.getRequest());
        }
        return LOCAL_IP;
    }
    ...
    public static HttpServletRequest getRequest() {
        try {
            ...
            throw new BusinessException(ApplicationStatus.ILLEGAL_ACCESS.getCode(), ExceptionUtils.getInfo(ex));
        }
    }
    ...
    public static long getSpentTime() {
        if (!RequestUtils.isServletContext()) return 0L;
        Object time = RequestUtils.getRequest().getAttribute(AttributeInfo.SPENT_TIME);
        return Objects.nonNull(time) ? Long.parseLong(String.valueOf(time)) : 0L;
    }
}
