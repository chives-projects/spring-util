...
public class LogbackAsyncAppender {
    ...
    public AsyncAppender getAppender(Appender ref) {
        ...
        appender.setDiscardingThreshold(this.getProperties().getAppender().getAsync().getDiscardingThreshold());
        ...
        appender.setMaxFlushTime(this.getProperties().getAppender().getAsync().getMaxFlushTime());
        //默认为 false，在队列满的时候 appender 会阻塞而不是丢弃信息。设置为 true，appender 不会阻塞你的应用而会将消息丢弃
        appender.setNeverBlock(this.getProperties().getAppender().getAsync().isNeverBlock());
